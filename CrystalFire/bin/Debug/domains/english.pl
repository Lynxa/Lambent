...
action(Agent, create_auction(Auction, Item, OpeningPrice)) :-
	...
	effects([
		create(Auction, english_auction),
		...
		set(Auction, opening_price, OpeningPrice),
		set(Auction, highest_bid, OpeningPrice),
...
action(Agent, open_auction(Auction, Item, OpeningPrice)) :-
	...
	transaction([
		...
		issue_p(right(place_bid(Auction, Bid), (
				value(Auction, highest_bid, HighestBid),
...
action(Agent, close_auction(Auction)) :-
